    ...
    public class EmpleadoRepository
    {
        public List<Empleado> ObtenerTodos()
        {
            ...
            string sql = @"SELECT Id, Nombre, Apodo, Cedula, Telefono, Rol, 
                                 SalarioBase, PorcentajeComision, FechaIngreso, VentasMes, Activo
                          FROM Empleados 
                          WHERE Activo = 1 
                          ORDER BY Nombre";
            ...
            using (SqlConnection conn = Conexion.ObtenerConexion())
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        typeof(Empleado).GetProperty("Id").SetValue(e, reader.GetInt32(0));
                        ...
                        e.VentasMes = reader.GetDecimal(9);
                        e.Activo = reader.GetBoolean(10);
                        lista.Add(e);
                    }
                }
            }
